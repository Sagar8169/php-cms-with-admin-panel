...
if (!empty($category)) {
    ...
    $count_stmt = $conn->prepare("SELECT COUNT(*) as total FROM posts WHERE status = 'published' AND category = ?");
    ...
    $stmt = $conn->prepare("SELECT posts.*, authors.name AS author_name 
                           ...
                           LIMIT ? OFFSET ?");
    $stmt->bind_param("sii", $category, $posts_per_page, $offset);
    $stmt->execute();
    $posts_result = $stmt->get_result();
}

// Get all categories for sidebar
$categories = $conn->query("SELECT category, COUNT(*) as count FROM posts WHERE status = 'published' AND category IS NOT NULL AND category != '' GROUP BY category ORDER BY count DESC");
...
<head>
    <meta charset="UTF-8">
    <title><?= !empty($category) ? 'Category: ' . htmlspecialchars($category) : 'Categories' ?> - My CMS Portal</title>
...
<div class="container">
    ...
    <div class="page-header">
        <?php if (!empty($category)): ?>
            <h1 class="page-title"><i class="fas fa-folder-open"></i> Category: <?= htmlspecialchars($category) ?></h1>
            <p class="page-description">
                Showing <?= $total_posts ?> post<?= $total_posts != 1 ? 's' : '' ?> in the "<?= htmlspecialchars($category) ?>" category
            </p>
        <?php else: ?>
            <h1 class="page-title"><i class="fas fa-folder"></i> All Categories</h1>
            <p class="page-description">Browse posts by category to discover content that interests you</p>
        <?php endif; ?>
    </div>

    <div class="main-container">
        <div class="content-area">
            <?php if (!empty($category) && $posts_result && $posts_result->num_rows > 0): ?>
                        ...
                        <div class="post-content">
                            ...
                            <div class="post-meta">
                                ...
                                <?php if (!empty($post['tags'])): ?>
                                    <span><i class="fas fa-tags"></i> <?= htmlspecialchars(str_replace(',', ', ', $post['tags'])) ?></span>
                                <?php endif; ?>
                            </div>
                ...
                <?php if ($total_pages > 1): ?>
                    <div class="pagination">
                        <?php if ($page > 1): ?>
                            <a href="?cat=<?= urlencode($category) ?>&page=<?= $page - 1 ?>">
                        ...
                        <?php for ($i = max(1, $page - 2); $i <= min($total_pages, $page + 2); $i++): ?>
                            ...
                            <?php else: ?>
                                <a href="?cat=<?= urlencode($category) ?>&page=<?= $i ?>"><?= $i ?></a>
                            <?php endif; ?>
                        <?php endfor; ?>
            ...
            <?php elseif (!empty($category)): ?>
                <div class="no-posts">
                    ...
                    <p>No posts were found in the "<?= htmlspecialchars($category) ?>" category.</p>
                    <p><a href="index.php"><i class="fas fa-arrow-left"></i> Back to Home</a></p>
                </div>
            <?php else: ?>
                <div class="no-posts">
                    <h2><i class="fas fa-folder"></i> Select a Category</h2>
                    <p>Choose a category from the sidebar to view related posts and discover interesting content.</p>
                </div>
            <?php endif; ?>
        </div>
        ...
        <aside class="sidebar">
            ...
            <?php if ($categories->num_rows > 0): ?>
                <ul class="category-list">
                    <?php while ($cat = $categories->fetch_assoc()): ?>
                        <li class="category-item <?= ($category === $cat['category']) ? 'active' : '' ?>">
            ...
            <?php else: ?>
                <p style="text-align: center; color: #777; font-style: italic;">No categories found.</p>
            <?php endif; ?>
        </aside>
    </div>
</div>
