...
if (!empty($tag)) {
    ...
    $count_stmt = $conn->prepare("SELECT COUNT(*) as total FROM posts WHERE status = 'published' AND tags LIKE ?");
    $count_stmt->bind_param("s", $tag_search);
    $count_stmt->execute();
    ...
    $stmt = $conn->prepare("SELECT posts.*, authors.name AS author_name 
                           ...
                           WHERE posts.status = 'published' AND posts.tags LIKE ?
                           ORDER BY posts.created_at DESC 
                           LIMIT ? OFFSET ?");
    $stmt->bind_param("sii", $tag_search, $posts_per_page, $offset);
    $stmt->execute();
    $posts_result = $stmt->get_result();
}

// Get all tags for sidebar
$all_tags = $conn->query("SELECT tags FROM posts WHERE status = 'published' AND tags IS NOT NULL AND tags != ''");
$tag_counts = array();

while ($tag_row = $all_tags->fetch_assoc()) {
    $tags = explode(',', $tag_row['tags']);
    foreach ($tags as $single_tag) {
        $single_tag = trim($single_tag);
        if (!empty($single_tag)) {
            $tag_counts[$single_tag] = isset($tag_counts[$single_tag]) ? $tag_counts[$single_tag] + 1 : 1;
        }
    }
}
...
<head>
    <meta charset="UTF-8">
    <title><?= !empty($tag) ? 'Tag: ' . htmlspecialchars($tag) : 'Tags' ?> - My CMS Portal</title>
...
<div class="container">
    ...
    <div class="page-header">
        <?php if (!empty($tag)): ?>
            <h1 class="page-title"><i class="fas fa-tag"></i> Tag: <?= htmlspecialchars($tag) ?></h1>
            <p class="page-description">
                Showing <?= $total_posts ?> post<?= $total_posts != 1 ? 's' : '' ?> tagged with "<?= htmlspecialchars($tag) ?>"
            </p>
        <?php else: ?>
            <h1 class="page-title"><i class="fas fa-tags"></i> All Tags</h1>
            <p class="page-description">Browse posts by tag to discover content that interests you</p>
        <?php endif; ?>
    </div>

    <div class="main-container">
        <div class="content-area">
            <?php if (!empty($tag) && $posts_result && $posts_result->num_rows > 0): ?>
                                ...
                                <div class="post-tags">
                                    ...
                                    <?php
                                        ...
                                        foreach ($post_tags as $post_tag) {
                                            ...
                                            $highlight_class = (strtolower($post_tag) === strtolower($tag)) ? 'highlight' : '';
                                            echo '<span class="' . $highlight_class . '">' . htmlspecialchars($post_tag) . '</span>';
                                        }
                                    ?>
                                </div>
                            <?php endif; ?>
                ...
                <?php if ($total_pages > 1): ?>
                    <div class="pagination">
                        <?php if ($page > 1): ?>
                            <a href="?tag=<?= urlencode($tag) ?>&page=<?= $page - 1 ?>">
            ...
            <?php elseif (!empty($tag)): ?>
                <div class="no-posts">
                    <h2><i class="fas fa-search"></i> No Posts Found</h2>
                    <p>No posts were found with the tag "<?= htmlspecialchars($tag) ?>".</p>
                    <p><a href="index.php"><i class="fas fa-arrow-left"></i> Back to Home</a></p>
                </div>
            <?php else: ?>
                <div class="no-posts">
                    <h2><i class="fas fa-tag"></i> Select a Tag</h2>
                    <p>Choose a tag from the sidebar to view related posts and discover interesting content.</p>
                </div>
            <?php endif; ?>
        </div>
        ...
        <aside class="sidebar">
            ...
            <?php if (!empty($tag_counts)): ?>
                <div class="tag-cloud">
                    <?php foreach ($tag_counts as $sidebar_tag => $count): ?>
                        <a href="tag.php?tag=<?= urlencode($sidebar_tag) ?>"
                           <?= (strtolower($tag) === strtolower($sidebar_tag)) ? 'class="active"' : '' ?>>
                            <?= htmlspecialchars($sidebar_tag) ?>
                            <span class="tag-count">(<?= $count ?>)</span>
                        </a>
                    <?php endforeach; ?>
                </div>
            <?php else: ?>
                <p style="text-align: center; color: #777; font-style: italic;">No tags found.</p>
            <?php endif; ?>
        </aside>
    </div>
</div>
